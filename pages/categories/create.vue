...
<template>
  ...
  <div class="border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg bg-white/60 dark:bg-gray-900/80 max-w-xl mx-auto p-4">
    <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        <UFormField label="Category Name" name="name" class="w-full">
        ...
        <UFormField label="Category Image" name="image" class="w-full">
            <UInput type="file" variant="subtle" size="xl" />
        </UFormField>
        

        <div class="flex items-center space-x-2">
            <UButton type="reset" to="/categories">Cancel</UButton>
            <UButton type="submit">Save Category</UButton>
        </div>
    </UForm>
  </div>
</template>
